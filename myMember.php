...
ini_set("display_errors",1);
  class myMember extends memberLog{
    ...
    function signUp(){
      ...
      $sql .= "VALUES('{$userNmae}', '{$userEmail}', '{$userPw}', '{$birth}', '{$gender}','{$profilePhoto}', '{$coverPhoto}', {$regTime})";
    ...
    //이메일 중복 체크 메소드
     function emailCheck($email){
      ...
      if(filter_var($email, FILTER_VALIDATE_EMAIL)){
        //같은 이메일 주소 있는지 찾는 쿼리 문
        $sql = "SELECT * FROM mymember WHERE email = '{$email}'";
        $this->dbConnection();
        $res = $this->dbConnection->query($sql);
        //데이터베이스에서 가져온 결과 수를 체크하여 0이면 사용가능, 0이 아니면 이미 존재하는 이메일
        if($res->num_rows == 0){
          $result = true;
        }
      }
     ...
     function phooSave(){
       ...
       //프로필 포토 업로드인지 확인
        is(isset($_FILES['myProfilePhoto'])){
        ...
        //허용하는 확장자가 맞는지 확인
        //허용할 확장자를 jpg, bmp, gif, png로 정하고, 그 외는 업로드 불가
        ...
        //저장할 파일명을 생성 ex) $uploadPhotoFolder861225.jpeg
